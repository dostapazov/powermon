...
void    ZrmCellView::channel_param_changed(unsigned channel, const zrm::params_list_t & params_list  )
{
    if(channel == m_channel)
    {
        ...
        if((ptr = params_list.find(zrm::PARAM_CON)  ) != end)
             connection_change();
...
void    ZrmCellView::connection_change    ()
{
   zrm::session_t sess = m_source->channel_session(m_channel);
   if(sess.is_active())
   {
     m_source->channel_subscribe_param(m_channel, zrm::PARAM_CCNT, true);
     m_source->channel_subscribe_param(m_channel, zrm::PARAM_CELL, true);
   }
}
...
void    ZrmCellView::cell_params          (uint16_t value)
{
  ...
  while(ptr<end)
  {
    ...
    {
     double volt = ptr->volt();
     item_u->setText(number_text(volt,1));
