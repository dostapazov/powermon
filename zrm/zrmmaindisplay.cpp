...
void  ZrmMainDisplay::update_controls()
{
	if (m_source && m_channel)
	{
		...
		setup_method();
		bool value = m_source->channel_work_mode(m_channel) == zrm::zrm_work_mode_t::charger;
		...
		for (auto param : params_list)
		{
			...
			switch (param.first)
			{
				...
				case zrm::PARAM_WTIME        :
					lb_work_time ->setText(value.toString());
					break;
				case zrm::PARAM_LTIME        :
					edTimeLimit->setText(value.toString());
					break;
				case zrm::PARAM_CUR          :
...
void ZrmMainDisplay::set_method_duration(zrm::zrm_method_t& method, const QString& str)
{
	...
	for (auto text : sl)
	{
		...
		{
			case 0:
				method.m_method.m_hours   = uint8_t(text.trimmed().toUInt());
				break;
			case 1:
				method.m_method.m_minutes = uint8_t(text.trimmed().toUInt());
				break;
			case 2:
				method.m_method.m_secs    = uint8_t(text.trimmed().toUInt());
				break;
			default :
				return;
		}
