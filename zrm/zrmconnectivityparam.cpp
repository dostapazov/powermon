...
QTreeWidgetItem * ZrmConnectivityParam::create_channel_item(zrm::ZrmConnectivity * conn, QTreeWidgetItem * conn_item, uint16_t chan_number)
{
 if(conn && conn_item)
 {
    if(conn_item->parent()) conn_item = conn_item->parent();
    ...
    chann_item->setData(0,Qt::UserRole, wm);
...
void ZrmConnectivityParam::update_tool_buttons(QTreeWidgetItem * item)
{
    ...
    if(conn_dev)
    {
     start_enabled = !conn_dev->is_working();
     stop_enabled  = !start_enabled;
    }
}
...
void   ZrmConnectivityParam::setup_channel           (QTreeWidgetItem * item)
{
  ...
  gb_masakb->setVisible(idx);
  if(idx)
  {
    auto conn = connectivity(item);
    auto _map = conn->channel_masakb_param(channel_number((item)));
    cell_dU->setValue(_map.dU);
    cell_dT->setValue(_map.dT);
  }
}
