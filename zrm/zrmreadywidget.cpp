...
ZrmChannelMimimal * ZrmReadyWidget::create_channel_widget()
{
  ...
  connect(w, &ZrmChannelMimimal::mouse_over, this, &ZrmReadyWidget::zrm_mouse_over);
  return w;
}
...
void  ZrmReadyWidget::set_layout_count(int count)
{
  ...
  while(count != ready_count)
  {
    ...
    if(ready_count > count)
    {
      ...
      auto w = litem->widget();
      if(m_current == w) m_current = Q_NULLPTR;
      delete w;
      delete litem;
    }
  }
...
void ZrmReadyWidget::zrm_chanhel_activate (ZrmChannelMimimal * w)
{
    if(m_current != w )
    {
      if(m_current)
    ...
    if(m_current) m_current->set_active(true);
    emit channel_activated(m_current ? m_current->connectivity() : Q_NULLPTR, m_current ? m_current->channel() : 0);
}
...
void ZrmReadyWidget::zrm_mouse_over ()
{
  auto w = dynamic_cast<ZrmChannelMimimal *>(sender());
  emit channel_mouse_over(w);
}
...
void ZrmReadyWidget::next_channel    ()
{
   if(m_ready_layout)
   {
       ...
       if(++idx >= m_ready_layout->count()   )
            idx = 0 ;
     zrm_chanhel_activate(dynamic_cast<ZrmChannelMimimal*>(m_ready_layout->itemAt(idx)->widget()));
   }
}
