...
void  ZrmMethodBase::channel_param_changed(unsigned channel, const zrm::params_list_t & params_list  )
{
  if(channel == m_channel)
  {
    for(auto param : params_list)
    {
        switch (param.first)
        {
          case zrm::PARAM_STG_NUM       :  set_current_stage( param.second.sword);  break;
          case zrm::PARAM_WTIME         :  lb_time_work->setText(param_get(param.first).toString());break;
          case zrm::PARAM_LOOP_NUM      :  set_number_value(lb_cycle_num, param.second.sword,2, no_value ) ;break;
