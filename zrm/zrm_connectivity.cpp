...

{
 ...
 if(!conn_name.isEmpty()) setObjectName(conn_name);
...
bool     ZrmConnectivity::set_connection_string(const QString & conn_str)
{
  ...
  if(ret) ++m_connectivities_changed;
  return ret;
}
...
void   ZrmConnectivity::module_state_changed (zrm_module_ptr_t & mod  , bool * pneed_request_method, bool * pneed_ping)
{
  ...
  if(ch.state_bits.auto_on || ch.state_bits.start_pause )
  {
    ...
    {
     if(need_ping) ping_module(mod.data());
    }
  }
...
void    ZrmConnectivity::channel_control_event(QChannelControlEvent * ctrl_event)
{
    ...
    {
        case ctrl_write_param   : channel_write_param  (ctrl_event->channel(),ctrl_event->wr_mode(), ctrl_event->param(), ctrl_event->data().constData(), ctrl_event->data_size());
                                  break;
        case ctrl_request_param : channel_query_params (ctrl_event->channel(),ctrl_event->data().constData(), size_t(ctrl_event->data().size()));
                                  break;
        default: break;
    }
  ctrl_event->accept();
...
QVariant     ZrmConnectivity::param_get( zrm::zrm_param_t param , const zrm::param_variant & pv)
{
  QVariant res;
    if(pv.is_valid())
    {
      switch(param)
      {
          case zrm::PARAM_STATE      : res =  pv.uword; break;
          case zrm::PARAM_WTIME      :
          case zrm::PARAM_LTIME      : res = QString::fromStdString(ZrmModule::time_param(pv)); break;
          ...
          case zrm::PARAM_TCONV      :res =  value(pv.value<double>(true),3); break;
          ...
          case zrm::PARAM_MID        : res = (pv.udword);break;
          case zrm::PARAM_TEMP       :
          case zrm::PARAM_TRECT      : res = QString::fromStdString( ZrmModule::trect_param(pv) );break;

          default: res = (pv.udword);break;
      }
    }
  return res;
}
  ...
  if(file.open(QFile::ReadOnly))
  {
      ...
      while(!file.atEnd())
      {
          ...
          if(sl.count()>1)
          {
             ...
             if(!dest.contains(idx))  dest[idx] = sl[1].trimmed();
          }
      }
  }
}
...
QString      ZrmConnectivity::zrm_work_mode_name( zrm_work_mode_t  wm)
{
 ...
 switch(wm)
 {
   case as_power   : ret = tr("Источник");break;
   case as_charger : ret = tr("Зарядное");break;
 }
 return ret;
}
...
{
    if(!m_mode_text.count()) load_text(QLatin1String(":/zrm/files/zrm_modes.txt"), m_mode_text);
...
QString      ZrmConnectivity::zrm_error_text  (uint32_t code)
{
    if(code)
    {
    if(!m_error_text.count()) load_text(QLatin1String(":/zrm/files/zrm_errors.txt"), m_error_text);
...
{
    if(!m_warning_text.count()) load_text(QLatin1String(":/zrm/files/zrm_warnings.txt"), m_warning_text);
