...
union session_t
{
 ...
 {
  session_param.mode = a_mode ;session_param.error = (a_error); session_param.ssID = (id);
 }
...
param_variant init_variant(T value)
{
    ...
    if(pv.size> sizeof(pv.puchar) ) pv.size = sizeof (pv.puchar);
...
T    param_variant::value(bool as_signed) const
{
   ...
   switch(size)
   {
    case sizeof(int8_t ) : res = as_signed ? T(sbyte)  : T(ubyte); break;
    case sizeof(int16_t) : res = as_signed ? T(sword)  : T(uword); break;
    case sizeof(int32_t) : res = as_signed ? T(sdword) : T(udword); break;
    case sizeof(int64_t) : res = as_signed ? T(sqword) : T(uqword); break;
    default : break;
   }
   return res;
}
