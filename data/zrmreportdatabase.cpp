...
bool ZrmReportDatabase::report_update       (QSqlDatabase & db, const QVariant& rep_id, uint32_t total_duration, qreal total_energy, qreal total_capacity)
{
  ...
  if(rep_query.prepare(qtext))
  {
    ...
    rep_query.bindValue(":total_capacity", total_capacity);
    return rep_query.exec();
  }
  return false;
}

bool  ZrmReportDatabase::report_write         (int user_id, int serial_number_id, const zrm::method_exec_results_t & results )
{
