...
bool ZrmMethodEditor::write_changes     (QTreeWidgetItem * item)
{
  ...
  if(ret)
  {
    ...
    {
      case ZrmMethodsTree::table_types : ret = write_type  ( item );break;
      case ZrmMethodsTree::table_models: ret = write_model ( item ) ;break;
      case ZrmMethodsTree::table_method: ret = write_method( item );break;
      default:  break;
    }
  }
...
bool ZrmMethodEditor::erase_types         (QTreeWidgetItem *item)
{
     ...
     )
  {
      ...
      if(ret)
      {
        ...
        if(ret) ZrmDatabase::commit_transaction(db);
      }
  }
  return ret;
}
...
void     ZrmMethodEditor::abstract_method_changed    (QTreeWidgetItem * current, QTreeWidgetItem * prev)
{
  ...
  if(current)
  {
    ...
    QTreeWidgetItem * dest_item = methods_tree->current_item();
...
void     ZrmMethodEditor::link_abstract_method ()
{
   ...
   QTreeWidgetItem * dest_item = this->methods_tree->current_item();
   ...
   {
     QTreeWidgetItem * method_item = methods_abstract->current_item();
