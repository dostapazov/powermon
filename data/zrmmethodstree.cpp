    ...
    QWidget(parent)
{
    ...
    connect(tw_methods, &QTreeWidget::itemExpanded      , this, &ZrmMethodsTree::slot_item_expanded );
    connect(tw_methods, &QTreeWidget::itemCollapsed     , this, &ZrmMethodsTree::slot_item_collapsed);
    connect(tw_methods, &QTreeWidget::currentItemChanged, this, &ZrmMethodsTree::current_item_changed);
    connect(tw_methods, &QTreeWidget::itemChanged       , this, &ZrmMethodsTree::item_changed);
    tw_methods->setFont(font());
}
...
bool     ZrmMethodsTree::open_database (zrm::zrm_work_mode_t work_mode, bool _abstract_methods)
{
 ...
 {
   close_database();
   auto hdr = tw_methods->headerItem();
   hdr->setText(column_capacity, work_mode ? tr("Ёмкость"):tr("Ток"));
   ...
   header->setText(2, work_mode ? tr("Ёмкость") : tr("Ток"));
   if(db.isOpen())
   {
    fill_tree();
   }
 ...
 if(db.isOpen() && !tw_methods->topLevelItemCount())
     fill_tree();
...
void  ZrmMethodsTree::save_user_values()
{
  if(db.isOpen() && this->m_abstract_methods)
  {
   bool ret = true;
   QSignalBlocker sb(tw_methods);
   for(int i = 0; ret && i < tw_methods->topLevelItemCount();i++)
   {
     QTreeWidgetItem * item = tw_methods->topLevelItem(i);
     ...
     if( it == table_method && uc.toInt())
     {
       ...
       if(ret) item->setData(column_name, role_user_changed,0);
     }
   }
   if(ret)
...
{
  return read_stages(tw_methods->currentItem(), stages);
}
...
void ZrmMethodsTree::on_tw_methods_itemChanged(QTreeWidgetItem *item, int column)
{
  if(column )
  {
 ...
 */
QTreeWidgetItem * ZrmMethodsTree::search_method_by_id(QTreeWidgetItem * item, const QVariant& method_id)
{
  ...
  {
    if(m_abstract_methods)
     result   = find_method_by_id<QTreeWidget>(tw_methods,method_id.toUInt(), &QTreeWidget::topLevelItemCount, &QTreeWidget::topLevelItem);
  }

 return result;
}

void ZrmMethodsTree::on_tw_methods_currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *previous)
{
    Q_UNUSED(previous)
//#ifdef Q_OS_ANDROID
    ...
    qApp->processEvents();
//#endif
    emit method_selected( current && current->data(column_name, role_table).toInt() == table_method ? current : Q_NULLPTR );
}
...
{
   auto * item = new QTreeWidgetItem(new_parent);
   item->setFlags(item->flags()|Qt::ItemFlag::ItemIsEditable);
...
QAbstractItemView::SelectionMode      ZrmMethodsTree::multi_selection()
{
 return this->tw_methods->selectionMode();
}

void      ZrmMethodsTree::multi_selection_enable(QAbstractItemView::SelectionMode mode)
{
  tw_methods->setSelectionMode( mode );
}
...
void      ZrmMethodsTree::show_method_params(bool show)
{
  if(show)
  {
    tw_methods->showColumn( column_voltage  );
    tw_methods->showColumn( column_capacity );
  }
  else
  {
    tw_methods->hideColumn( column_voltage  );
    tw_methods->hideColumn( column_capacity );
  }
}
...
{
    auto src = sender();
    auto item = tw_methods->currentItem();
//    qDebug()<<Q_FUNC_INFO<<src<<val_text;
    item->setText(tw_methods->currentColumn(), val_text);
