...
bool          ZrmDatabase::read_method       (QSqlDatabase & db, const QVariant &id, zrm::method_t & result, double voltage, double capacity)
{
  ...
  if(skip_empty(query))
  {
     ...
     if(qFuzzyIsNull(voltage)) voltage = 1.0;
...
QSqlQuery     ZrmDatabase::read_method_stages(QSqlDatabase & db, const QVariant &m_id)
{
    QSqlQuery query(db);
...
bool          ZrmDatabase::read_method_stages(QSqlDatabase & db,const QVariant & m_id, zrm::stages_t & result)
{
...
QString   ZrmDatabase::read_stage_descript(QSqlDatabase & db, const QVariant &stage_id)
{
  QSqlQuery query(db)   ;
...
bool  ZrmDatabase::write_stage_descript(QSqlDatabase & db, const QVariant & stage_id, const QString & stage_descript)
{
...
bool ZrmDatabase::link_method(QSqlDatabase & db, const QVariant &method_id, const QVariant &model_id)
{
...
bool ZrmDatabase::unlink_method(QSqlDatabase & db, const QVariant & method_id, const QVariant & model_id)
{
    start_transaction(db);
    query_args_t args;
    QString qtext =
...
bool  ZrmDatabase::erase_stages      (QSqlDatabase & db, zrm::stages_t & stages)
{
  ...
  {
    ret &= erase_stage(db,st.m_id_method);
    if(!ret)
        break;
                                      ...
                                      )
{
 ...
 :
 "UPDATE t_method SET "
 "c_name = :name, n_volt_rate = :volt_rate, n_current_rate = :current_rate,  n_duration = :duration,  n_cycle_count = :cycle_count "
 "WHERE id = :id"
             ;
query_args_t args;
if(!is_new)
...
bool ZrmDatabase::write_stage       (QSqlDatabase & db, QVariant & method_id, zrm::stage_t  & stage)
{
 ...
 QString ins_text =
     ...
     bool is_new = !stage.m_id_method;
     query_args_t args;
     if( !is_new )
     {
         args[":id"] = stage.m_id_method;
     }
     else
     {
         args[":id_method"] = method_id;
       ...
       if(ret && is_new)
          {
            stage.m_id_method = uint16_t(query.lastInsertId().toUInt());
          }
 return ret;
}
